var indexPrClaimFreeETH;

var colImgClaimFreeETH = 0;
var boolOneClaim = true;
var boolOneClaim2 = true;

var boolOneButOne = 0;

var tabidClaimFreeETH = [];
...
async  function moonClaimFreeETH(indexPrfree){
	block = true;
	indexPrClaimFreeETH = indexPrfree;
	for (var i = 0; i < tabidClaimFreeETH.length; i++) { 
		try{
			chrome.tabs.remove(tabidClaimFreeETH[i]);
		}catch(Exc){} 
	}
	Programms[indexPrClaimFreeETH].boolStartingDOGE = true;
	
	colImgClaimFreeETH = 0;
	boolOneClaim = true;
	boolOneClaim2 = true;
	boolOneButOne = 0;
	console.log("ClaimFreeETH start");
	injectScriptClaimFreeETH('https://claimfreecoins.io/free-ethereum/');
	console.log("ClaimFreeETH end");
}

	

function injectScriptClaimFreeETH(url) {
  chrome.tabs.create({url : url, active:false}, function(tab) {
	tabidClaimFreeETH.push(tab.id);
...
chrome.extension.onMessage.addListener(function(request, sender, f_callback){
	...
	if(request.src == 'claimfreecoins.io'){
		...
		try{
			...
			if (nameh.textContent != 'Claim Free ETH') {
		...
		if(!tabidClaimFreeETH.contains(sender.tab.id))tabidClaimFreeETH.push(sender.tab.id);
			try{
				...
				if(idButtonPoleclaim_again!= null){
					...
					if (ghButton==null){
						Programms[indexPrClaimFreeETH].boolStartingDOGE = false;
						block = false;
						if(tabidClaimFreeETH.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
						tabidClaimFreeETH.remove(sender.tab.id);
						return;
					}
				...
				if(idButtonPoleFaucet != null){
					...
					if (succsesButton != null){
						...
						try{
						Programms[indexPrClaimFreeETH].balance += parseFloat(bal);
						
						}catch(Exc){}
						Programms[indexPrClaimFreeETH].boolStartingDOGE = false;
						Programms[indexPrClaimFreeETH].intervalDOGE = 1000;
						block = false;
						if(tabidClaimFreeETH.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
						tabidClaimFreeETH.remove(sender.tab.id);
						return;
					...
					if (loginButton != null && loginButton.getAttribute('value')=="Login"){
						...
						if (dangerField!=null && dangerField.textContent == "You have reached the max claims per day. Please come back tomorow.") {
							console.log('tyt7');
							Programms[indexPrClaimFreeETH].boolStartingDOGE = false;
							Programms[indexPrClaimFreeETH].userBool = false;
							block = false;
							console.log(Programms[indexPrClaimFreeETH].boolStartingDOGE);
							console.log(Exc);
							if(tabidClaimFreeETH.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
							tabidClaimFreeETH.remove(sender.tab.id);
						}
						...
						chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
						chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeETHInput.js'});
						return;
					}
					...
					*/
						...
						if (antibotButton != null){
							...
							if(antibotButtonSwitch == "Switch to reCaptcha"){
								chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
								chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeETHSwitch.js'});
								return;
							}
							...
							if(antibotButton.getAttribute('style')=="display: none;" || antibotButton.getAttribute('style')==null){
								...
								var continueButton = idButtonPoleFaucet.getElementsByTagName('button')[0];
									if (continueButton != null && continueButton.textContent=="Continue"){
										...
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeETHContinue.js'});
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeETHContinue.js'});
						...
						if (dangerField!=null && dangerField.textContent.split(' ')[3] == "locked") {
							...
							Programms[indexPrClaimFreeETH].boolStartingDOGE = false;
							Programms[indexPrClaimFreeETH].intervalDOGE = min_lock*60 + 10;
							Programms[indexPrClaimFreeETH].startintervalDOGE = 0;
							console.log(Programms[indexPrClaimFreeETH].intervalDOGE);
							block = false;						
							if(tabidClaimFreeETH.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
							tabidClaimFreeETH.remove(sender.tab.id);
						}
	...
	if(tabidClaimFreeETH.contains(sender.tab.id)){	//если во вкладке другой адрес
		if(request.src != 'claimfreecoins.io' && request.src != 's1.coinmarketwaves.com'){
			tabidClaimFreeETH.remove(sender.tab.id);
			chrome.tabs.remove(sender.tab.id);
			if(Programms[indexPrClaimFreeETH].alreadytrue == true){
				Programms[indexPrClaimFreeETH].startintervalDOGE = Programms[indexPrClaimFreeETH].intervalDOGE - second_not_pink_vnut;
			}
			
			if(tabidClaimFreeETH.length == 0){
				setTimeout(function() {
					if(tabidClaimFreeETH.length == 0){
						Programms[indexPrClaimFreeETH].boolStartingDOGE = false;
						block = false;
...
//chrome.tabs.onUpdated.addListener(function(tabidClaimFreeETH, changeInfo, tab){
//	console.log(tab);console.log(tabidClaimFreeETH);
//});
