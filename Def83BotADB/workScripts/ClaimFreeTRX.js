var indexPrClaimFreeTRX;

var colImgClaimFreeTRX = 0;
var boolOneClaim = true;
var boolOneClaim2 = true;

var boolOneButOne = 0;

var tabidClaimFreeTRX = [];
...
async  function moonClaimFreeTRX(indexPrfree){
	block = true;
	indexPrClaimFreeTRX = indexPrfree;
	for (var i = 0; i < tabidClaimFreeTRX.length; i++) { 
		try{
			chrome.tabs.remove(tabidClaimFreeTRX[i]);
		}catch(Exc){} 
	}
	Programms[indexPrClaimFreeTRX].boolStartingDOGE = true;
	
	colImgClaimFreeTRX = 0;
	boolOneClaim = true;
	boolOneClaim2 = true;
	boolOneButOne = 0;
	console.log("ClaimFreeTRX start");
	injectScriptClaimFreeTRX('https://claimfreecoins.io/free-tron/');
	console.log("ClaimFreeTRX end");
}

	

function injectScriptClaimFreeTRX(url) {
  chrome.tabs.create({url : url, active:false}, function(tab) {
	tabidClaimFreeTRX.push(tab.id);
...
chrome.extension.onMessage.addListener(function(request, sender, f_callback){
	...
	if(request.src == 'claimfreecoins.io'){
		...
		try{
			...
			if (nameh.textContent != 'Claim Free TRX') {
		...
		if(!tabidClaimFreeTRX.contains(sender.tab.id))tabidClaimFreeTRX.push(sender.tab.id);
			try{
				...
				if(idButtonPoleclaim_again!= null){
					...
					if (ghButton==null){
						Programms[indexPrClaimFreeTRX].boolStartingDOGE = false;
						block = false;
						if(tabidClaimFreeTRX.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
						tabidClaimFreeTRX.remove(sender.tab.id);
						return;
					}
				...
				if(idButtonPoleFaucet != null){
					...
					if (succsesButton != null){
						...
						try{
						Programms[indexPrClaimFreeTRX].balance += parseFloat(bal);
						
						}catch(Exc){}
						Programms[indexPrClaimFreeTRX].boolStartingDOGE = false;
						Programms[indexPrClaimFreeTRX].intervalDOGE = 1000;
						block = false;
						if(tabidClaimFreeTRX.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
						tabidClaimFreeTRX.remove(sender.tab.id);
						return;
					...
					if (loginButton != null && loginButton.getAttribute('value')=="Login"){
						...
						if (dangerField!=null && dangerField.textContent == "You have reached the max claims per day. Please come back tomorow.") {
							console.log('tyt7');
							Programms[indexPrClaimFreeTRX].boolStartingDOGE = false;
							Programms[indexPrClaimFreeTRX].userBool = false;
							block = false;
							console.log(Programms[indexPrClaimFreeTRX].boolStartingDOGE);
							console.log(Exc);
							if(tabidClaimFreeTRX.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
							tabidClaimFreeTRX.remove(sender.tab.id);
						}
						...
						chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
						chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeTRXInput.js'});
						return;
					}
					...
					*/
						...
						if (antibotButton != null){
							...
							if(antibotButtonSwitch == "Switch to reCaptcha"){
								chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
								chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeTRXSwitch.js'});
								return;
							}
							...
							if(antibotButton.getAttribute('style')=="display: none;" || antibotButton.getAttribute('style')==null){
								...
								var continueButton = idButtonPoleFaucet.getElementsByTagName('button')[0];
									if (continueButton != null && continueButton.textContent=="Continue"){
										...
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeTRXContinue.js'});
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeTRXContinue.js'});
						...
						if (dangerField!=null && dangerField.textContent.split(' ')[3] == "locked") {
							...
							Programms[indexPrClaimFreeTRX].boolStartingDOGE = false;
							Programms[indexPrClaimFreeTRX].intervalDOGE = min_lock*60 + 10;
							Programms[indexPrClaimFreeTRX].startintervalDOGE = 0;
							console.log(Programms[indexPrClaimFreeTRX].intervalDOGE);
							block = false;						
							if(tabidClaimFreeTRX.contains(sender.tab.id)&&!tabidSave.contains(sender.tab.id)) chrome.tabs.remove(sender.tab.id);
							tabidClaimFreeTRX.remove(sender.tab.id);
						}
	...
	if(tabidClaimFreeTRX.contains(sender.tab.id)){	//если во вкладке другой адрес
		if(request.src != 'claimfreecoins.io' && request.src != 's1.coinmarketwaves.com'){
			tabidClaimFreeTRX.remove(sender.tab.id);
			chrome.tabs.remove(sender.tab.id);
			if(Programms[indexPrClaimFreeTRX].alreadytrue == true){
				Programms[indexPrClaimFreeTRX].startintervalDOGE = Programms[indexPrClaimFreeTRX].intervalDOGE - second_not_pink_vnut;
			}
			
			if(tabidClaimFreeTRX.length == 0){
				setTimeout(function() {
					if(tabidClaimFreeTRX.length == 0){
						Programms[indexPrClaimFreeTRX].boolStartingDOGE = false;
						block = false;
...
//chrome.tabs.onUpdated.addListener(function(tabidClaimFreeTRX, changeInfo, tab){
//	console.log(tab);console.log(tabidClaimFreeTRX);
//});
