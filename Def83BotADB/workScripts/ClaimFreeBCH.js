var indexPrClaimFreeBCH;

var colImgClaimFreeBCH = 0;
var boolOneClaim = true;
var boolOneClaim2 = true;

var boolOneButOne = 0;

var tabidClaimFreeBCH = [];
...
async  function moonClaimFreeBCH(indexPrfree){
	block = true;
	indexPrClaimFreeBCH = indexPrfree;
	for (var i = 0; i < tabidClaimFreeBCH.length; i++) { 
		try{
			chrome.tabs.remove(tabidClaimFreeBCH[i]);
		}catch(Exc){} 
	}
	Programms[indexPrClaimFreeBCH].boolStartingDOGE = true;
	
	colImgClaimFreeBCH = 0;
	boolOneClaim = true;
	boolOneClaim2 = true;
	boolOneButOne = 0;
	console.log("ClaimFreeBCH start");
	injectScriptClaimFreeBCH('https://claimfreecoins.io/free-bitcoin-cash/');
	console.log("ClaimFreeBCH end");
}

	

function injectScriptClaimFreeBCH(url) {
  chrome.tabs.create({url : url, active:false}, function(tab) {
	tabidClaimFreeBCH.push(tab.id);
...
chrome.extension.onMessage.addListener(function(request, sender, f_callback){
	...
	if(request.src == 'claimfreecoins.io'){
		...
		try{
			...
			if (nameh.textContent != 'Claim Free BCH') {
		...
		if(!tabidClaimFreeBCH.contains(sender.tab.id))tabidClaimFreeBCH.push(sender.tab.id);
			try{
				...
				if(idButtonPoleFaucet != null){
					...
					if (succsesButton != null){
						...
						try{
						Programms[indexPrClaimFreeBCH].balance += parseFloat(bal);
					...
					*/
						...
						if (antibotButton != null){
							...
							if(antibotButtonSwitch == "Switch to reCaptcha"){
								chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
								chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeBCHSwitch.js'});
								return;
							}
							...
							if(antibotButton.getAttribute('style')=="display: none;" || antibotButton.getAttribute('style')==null){
								...
								var continueButton = idButtonPoleFaucet.getElementsByTagName('button')[0];
									if (continueButton != null && continueButton.textContent=="Continue"){
										...
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeBCHContinue.js'});
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/function.js'});
										//chrome.tabs.executeScript(sender.tab.id, {runAt:'document_end', file: 'content_scripts/UserScript/ClaimFreeBCHContinue.js'});
	...
	if(tabidClaimFreeBCH.contains(sender.tab.id)){	//если во вкладке другой адрес
		if(request.src != 'claimfreecoins.io' && request.src != 's1.coinmarketwaves.com'){
			tabidClaimFreeBCH.remove(sender.tab.id);
			chrome.tabs.remove(sender.tab.id);
			if(Programms[indexPrClaimFreeBCH].alreadytrue == true){
				Programms[indexPrClaimFreeBCH].startintervalDOGE = Programms[indexPrClaimFreeBCH].intervalDOGE - second_not_pink_vnut;
			}
			
			if(tabidClaimFreeBCH.length == 0){
				setTimeout(function() {
					if(tabidClaimFreeBCH.length == 0){
						Programms[indexPrClaimFreeBCH].boolStartingDOGE = false;
						block = false;
...
//chrome.tabs.onUpdated.addListener(function(tabidClaimFreeBCH, changeInfo, tab){
//	console.log(tab);console.log(tabidClaimFreeBCH);
//});
